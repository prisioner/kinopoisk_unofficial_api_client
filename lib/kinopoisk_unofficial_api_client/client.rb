  ...
  class Client
    ...
    def connection
      @connection ||= Faraday.new(url: url, headers: { "X-API-KEY" => secret_key }) do |faraday|
        faraday.adapter KinopoiskUnofficialApiClient.configuration.adapter
        faraday.options.timeout = KinopoiskUnofficialApiClient.configuration.connection_timeout
        faraday.options.open_timeout = KinopoiskUnofficialApiClient.configuration.connection_open_timeout
      end
    end
    ...
    def build_params(method_name, raw_params)
      ...
      schema = KinopoiskUnofficialApiClient::RequestSchemas.const_get(schema_name)
